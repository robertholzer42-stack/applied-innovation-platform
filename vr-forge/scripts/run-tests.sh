...
for PLATFORM in EditMode PlayMode; do
  RESULTS="Builds/test-results-${PLATFORM}.xml"
  LOG="Builds/test-${PLATFORM}.log"
  rm -f "$RESULTS"
  ...
  if [ ! -f "$RESULTS" ]; then
    ...
    FAIL_REASON="$PLATFORM tests did not run (Unity exit $UNITY_EXIT); see $LOG"
    break
  fi
  ...
  if [ "$FAILED_COUNT" -gt 0 ]; then
    ...
    FAIL_REASON="$PLATFORM tests failed; see $RESULTS"
    break
  fi
